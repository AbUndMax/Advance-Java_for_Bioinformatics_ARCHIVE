...
public class TreeLoader {
    ...
    private static ANode createTree(LinkedList<Relation> relations, HashMap<String, String> conceptIDtoRepID, HashMap<String, Collection<String>> fileList) {
        ...
        for (Relation relation : relations) {
            ...
            //get child/parent
            ANode parent = nodes.containsKey(parentID) ?
                    nodes.get(parentID) :
                    new ANode(parentID, conceptIDtoRepID.get(parentID), parentName, fileList.get(parentID));
            ANode child = nodes.containsKey(childID) ?
                    nodes.get(childID) :
                    new ANode(childID, conceptIDtoRepID.get(childID), childName, fileList.get(childID));
